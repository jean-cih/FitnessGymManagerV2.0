    ...
    public partial class ChangeArhiveService : ShadowedForm
    {
        ...
        public ChangeArhiveService()
        {
            try
            {
                ...
                Logger.Info("Форма ChangeArhiveService инициализирована");
            }
            catch (Exception ex)
            {
                Logger.Error("Ошибка при инициализации ChangeArhiveService", ex);
                throw;
            }
        }
        ...
        private void InitializeCustomDesign()
        {
            try
            {
                ...
                UIStyler.StyleTextBox(this, jeanTextBoxCard);
                ...
                btnClose.Click += (s, e) =>
                {
                    Logger.Info("Закрытие формы ChangeArhiveService");
                ...
                Logger.Info("Дизайн формы ChangeArhiveService инициализирован");
            }
            catch (Exception ex)
            {
        ...
        public void UpdateData()
        {
            try
            {
                ...
                Logger.Info($"Клиент: {jeanTextBoxClient.Text}, карта: {jeanTextBoxCard.Text}");
                Logger.Info($"Абонемент: {jeanTextBoxMembership.Text}, срок: {jeanTextBoxTerm.Text}, оплата: {jeanTextBoxCost.Text}, посещений: {jeanTextBoxVisits.Text}");
            }
            catch (Exception ex)
            {
        ...
        private void jeanModernButtonChange_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                Logger.Info($"Данные успешно обновлены в Archive для ID: {_id}");
                Logger.Info($"Новые данные: клиент={jeanTextBoxClient.Text}, карта={jeanTextBoxCard.Text}, " +
                    $"абонемент={jeanTextBoxMembership.Text}, срок={jeanTextBoxTerm.Text}, " +
                    $"оплата={jeanTextBoxCost.Text}, посещений={jeanTextBoxVisits.Text}");

                MessageHelper.ShowNotification(this, "✅ Данные обновлены", 1500);
                _fadeAnimation.CloseWithAnimation();

                Logger.Info($"Форма ChangeArhiveService закрыта после успешного обновления");
            }
            catch (Exception ex)
            {
                Logger.Error($"Ошибка при изменении данных в архиве для ID: {_id}", ex);
